...
회원가입 기능을 만들고 있다. 일단 sign-up 페이지 레이아웃만 만들어뒀는데
생각해야 할 점이 너무 많다. Validation도 제대로 이루어지지 않고 있다.
회원가입 기능을 완성시키고, to-do lists도 각 회원에 종속시켜서
자신이 만든 to-do lists만 볼 수 있도록 개선해야 한다.
...
다음에 할 일은 지금까지 한 것에 대해 테스트 코드 작성.